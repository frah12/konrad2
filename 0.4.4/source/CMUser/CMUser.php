 ...
 */
		...
		switch($action){
			case 'install' :
				try{
					...
					'Successfully created database and tables. Created default administrator as root:root, and user as doe:doe.');
				} catch(Exception $e){
	...
	public static function SQL($key=null) {
      ...
      'create table user2groups'=>"CREATE TABLE IF NOT EXISTS User2Groups (idUser INTEGER, idGroups INTEGER, created DATETIME default (datetime('now')), PRIMARY KEY(idUser, idGroups));",
		...
		'update password'=>"UPDATE User SET algorithm=?, salt=?, password=?, updated=datetime('now') WHERE id=?;"); // end queries array
